...
def _tmdb_tv_trailer_and_stills(title, year=None):
    ...
    # Fetch videos + images (videos fetched separately without language filter)
    details = _tmdb_get(f"/tv/{tmdb_id}", {"language": "en-US", "append_to_response": "images"})
    if not details:
        return None, [], None
    details["videos"] = _tmdb_get(f"/tv/{tmdb_id}/videos", {}) or {}
