...
FROM_EMAIL = os.environ.get("FROM_EMAIL", "recommendations@watchchums.com")
SITE_URL = os.environ.get("SITE_URL", "https://www.watchchums.com")
...
TELEPROMPTER_FILE = os.path.join(PROJECT_ROOT, "teleprompter.html")
_db_dir = os.path.dirname(os.environ.get("DB_PATH", os.path.join(os.path.dirname(os.path.abspath(__file__)), "filmshare.db")))
TELEPROMPTER_JSON = os.path.join(_db_dir, "teleprompter.json")
...
def _render_share_page(title="", year="", rating="", genre="", sender="A friend",
    ...
    # Replace OG tags with regex — robust against attribute spacing / self-close format
    html = _re.sub(
...
def _send_recommendation_email(to_email: str, to_name: str, from_name: str, film_title: str,
                               film_year, film_poster: str, note: str, rating):
    """Send an HTML email notification when someone sends an in-app recommendation."""
    if not SMTP_HOST or not SMTP_USER or not SMTP_PASS:
        return  # SMTP not configured — skip silently
    try:
        stars_filled = '&#9733;' * int(round(rating)) if rating else ''
        stars_empty = '&#9734;' * (5 - int(round(rating))) if rating else ''
        star_block = (
            f'<p style="font-size:22px;color:#E8C96A;margin:8px 0 0;">{stars_filled}'
            f'<span style="color:#444;">{stars_empty}</span></p>'
        ) if rating else ''
        note_block = (
            f'<blockquote style="border-left:3px solid #E8C96A;margin:16px 0;padding:8px 16px;'
            f'color:#ccc;font-style:italic;">&ldquo;{note}&rdquo;</blockquote>'
        ) if note else ''
        poster_block = (
            f'<img src="{film_poster}" alt="{film_title}" style="width:160px;border-radius:12px;'
            f'display:block;margin:16px auto;" />'
        ) if film_poster else ''
        year_str = f' ({film_year})' if film_year else ''
        html = f"""
        <div style="background:#0e0e14;color:#eae6dc;font-family:'Helvetica Neue',Arial,sans-serif;
                    max-width:520px;margin:0 auto;border-radius:16px;overflow:hidden;">
          <div style="background:#13131c;padding:28px 32px 20px;text-align:center;">
            <p style="color:#E8C96A;font-size:12px;letter-spacing:3px;text-transform:uppercase;
                      margin:0 0 6px;">Watch Chums</p>
            <h1 style="font-size:22px;margin:0;color:#fff;">{from_name} thinks you'll love this!</h1>
          </div>
          <div style="padding:24px 32px;text-align:center;">
            {poster_block}
            <h2 style="font-size:20px;font-weight:800;color:#fff;margin:12px 0 4px;">
              {film_title}{year_str}</h2>
            {note_block}
            {star_block}
            <a href="{SITE_URL}" style="display:inline-block;margin-top:24px;padding:14px 32px;
               background:#E8C96A;color:#000;font-weight:700;border-radius:999px;
               text-decoration:none;font-size:15px;">Open Watch Chums</a>
          </div>
          <div style="padding:16px 32px 24px;text-align:center;border-top:1px solid #1a1a26;">
            <p style="color:#5a566a;font-size:12px;margin:0;">
              You received this because {from_name} recommended a film to you on Watch Chums.</p>
          </div>
        </div>
        """
        msg = MIMEMultipart('alternative')
        msg['Subject'] = f"{from_name} has recommended a film to you on Watch Chums!"
        msg['From'] = f"Watch Chums <{FROM_EMAIL}>"
        msg['To'] = to_email
        msg.attach(MIMEText(
            f"{from_name} thinks you'll love {film_title}! Open Watch Chums: {SITE_URL}", 'plain'))
...
def _get_or_create_film(conn, title, year):
    """Look up film by title+year. If missing artwork, reenrich from TMDB. If not found, create it."""
    film = None
    if year:
        try:
            film = conn.execute("SELECT id, poster FROM films WHERE LOWER(title)=LOWER(?) AND year=?", (title, int(year))).fetchone()
        except Exception:
            pass
    if not film:
        film = conn.execute("SELECT id, poster FROM films WHERE LOWER(title)=LOWER(?)", (title,)).fetchone()

    if film and not film["poster"]:
        try:
            enriched = _tmdb.enrich_film(title, int(year) if year else None)
            if enriched.get("poster"):
                conn.execute(
                    "UPDATE films SET poster=COALESCE(poster,?), backdrop=COALESCE(backdrop,?), "
                    "genre=COALESCE(genre,?), description=COALESCE(description,?), "
                    "tmdb_id=COALESCE(tmdb_id,?), trailer_url=COALESCE(trailer_url,?) WHERE id=?",
                    (enriched.get("poster"), enriched.get("backdrop"), enriched.get("genre"),
                     enriched.get("description"), enriched.get("tmdb_id"), enriched.get("trailer_url"), film["id"])
                )
                for s in enriched.get("streamers", []):
                    conn.execute("INSERT OR IGNORE INTO film_streamers (film_id, streamer) VALUES (?,?)", (film["id"], s))
                for s in enriched.get("stills", []):
                    try:
                        conn.execute("INSERT INTO film_stills (film_id, still_url) VALUES (?,?)", (film["id"], s))
                    except Exception:
                        pass
        except Exception:
            pass
    ...
    if not film:
        try:
            ...
            cur = conn.execute(
                """INSERT INTO films (title, year, genre, runtime, director, "cast", rating, shared_by,
                   poster, backdrop, trailer_url, description, tmdb_id, slug)
                   VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (title, int(year) if year else enriched.get("year"), enriched.get("genre"),
                 enriched.get("runtime"), enriched.get("director"), enriched.get("cast"),
                 enriched.get("rating"), "A friend",
                 enriched.get("poster"), enriched.get("backdrop") or enriched.get("poster"),
                 enriched.get("trailer_url"), enriched.get("description"), enriched.get("tmdb_id"), slug)
            )
            fid = cur.lastrowid
            for s in enriched.get("streamers", []):
            ...
            for s in enriched.get("stills", []):
                try:
                    conn.execute("INSERT INTO film_stills (film_id, still_url) VALUES (?,?)", (fid, s))
                except Exception:
                    pass
            film = {"id": fid}
        except Exception:
        ...
        if req.invite_from_user:
            ...
            if inviter and inviter["id"] != user_id:
                ...
                if req.invite_film_title:
                    film = _get_or_create_film(conn, req.invite_film_title, req.invite_film_year)
                    if film:
                        conn.execute(
        ...
        if is_new_user and invite_from_user:
            ...
            if inviter and inviter["id"] != user["id"]:
                ...
                if invite_film_title:
                    film = _get_or_create_film(conn, invite_film_title, invite_film_year)
                    if film:
                        conn.execute(
                            "INSERT OR IGNORE INTO user_recommendations (film_id, from_user_id, to_user_id, note, rating) VALUES (?,?,?,?,?)",
...
def send_friend_request(username: str, current_user=Depends(require_user)):
    with get_db() as conn:
        ...
        if '@' in username and '.' in username.split('@', 1)[-1]:
            target = conn.execute("SELECT id FROM users WHERE LOWER(email) = LOWER(?)", (username,)).fetchone()
        else:
...
@app.post("/api/admin/set-streamers")
def admin_set_streamers(body: dict, token: str = ""):
    """Admin: set streaming services for a film by id or title."""
    _check_admin(token)
    ...
    with get_db() as conn:
        ...
        conn.execute("DELETE FROM film_streamers WHERE film_id=?", (film["id"],))
        for s in streamers:
            conn.execute("INSERT OR IGNORE INTO film_streamers (film_id, streamer) VALUES (?,?)", (film["id"], s))
    return {"ok": True, "film_id": film["id"], "streamers": streamers}


@app.get("/api/admin/film-usage")
def admin_film_usage(ids: str, token: str = ""):
    """Admin: check watchlist/watched/recommendation counts for film IDs (comma-separated)."""
    _check_admin(token)
    id_list = [int(i.strip()) for i in ids.split(",") if i.strip().isdigit()]
    with get_db() as conn:
        results = []
        for fid in id_list:
            film = conn.execute("SELECT id, title, year, trailer_url FROM films WHERE id=?", (fid,)).fetchone()
            if not film:
                continue
            wl = conn.execute("SELECT COUNT(*) as c FROM user_watchlist WHERE film_id=?", (fid,)).fetchone()["c"]
            wd = conn.execute("SELECT COUNT(*) as c FROM user_watched WHERE film_id=?", (fid,)).fetchone()["c"]
            rc = conn.execute("SELECT COUNT(*) as c FROM user_recommendations WHERE film_id=?", (fid,)).fetchone()["c"]
            results.append({"id": fid, "title": film["title"], "year": film["year"],
                            "trailer": bool(film["trailer_url"]), "watchlist": wl, "watched": wd, "recs": rc})
    return results


@app.delete("/api/admin/films/{film_id}")
def admin_delete_film(film_id: int, token: str = ""):
    """Admin: delete a film and all associated user data."""
    _check_admin(token)
    with get_db() as conn:
        film = conn.execute("SELECT id, title FROM films WHERE id=?", (film_id,)).fetchone()
        if not film:
            raise HTTPException(status_code=404, detail="Film not found")
        conn.execute("DELETE FROM user_watchlist WHERE film_id=?", (film_id,))
        conn.execute("DELETE FROM user_watched WHERE film_id=?", (film_id,))
        conn.execute("DELETE FROM user_ratings WHERE film_id=?", (film_id,))
        conn.execute("DELETE FROM user_recommendations WHERE film_id=?", (film_id,))
        conn.execute("DELETE FROM film_stills WHERE film_id=?", (film_id,))
        conn.execute("DELETE FROM film_streamers WHERE film_id=?", (film_id,))
        conn.execute("DELETE FROM films WHERE id=?", (film_id,))
    return {"ok": True, "deleted": film["title"], "id": film_id}
...
def get_my_ratings(current_user=Depends(require_user)):
    """Return all of the current user's ratings as {film_id: rating}."""
    with get_db() as conn:
        rows = conn.execute(
            "SELECT film_id, rating FROM user_ratings WHERE user_id=?",
            (current_user["id"],)
        ).fetchall()
    return {row["film_id"]: row["rating"] for row in rows}
...
def refresh_film(film_id: int):
    ...
    with get_db() as conn:
        ...
        if tmdb_id:
            ...
            # Always fetch watch providers for TMDB films — clear old values regardless
            providers = _tmdb.get_watch_providers(tmdb_id)
            conn.execute("DELETE FROM film_streamers WHERE film_id=?", (film_id,))
            for p in providers:
                conn.execute("INSERT OR IGNORE INTO film_streamers (film_id, streamer) VALUES (?,?)", (film_id, p))
        else:
        ...
        # YouTube fallback — if still no trailer, search YouTube API
        if not enriched.get("trailer_url") and not d.get("trailer_url"):
            yt_key = os.environ.get("YOUTUBE_API_KEY") or os.environ.get("YOUTUBE_KEY", "")
            if yt_key:
                try:
                    import urllib.request, urllib.parse, json as _json
                    q = urllib.parse.urlencode({"part": "snippet", "q": f"{title} {year or ''} official trailer", "type": "video", "maxResults": 1, "key": yt_key})
                    with urllib.request.urlopen(f"https://www.googleapis.com/youtube/v3/search?{q}", timeout=10) as r:
                        data = _json.loads(r.read())
                    items = data.get("items", [])
                    if items:
                        video_id = items[0]["id"].get("videoId")
                        if video_id:
                            enriched["trailer_url"] = f"https://www.youtube.com/embed/{video_id}"
                except Exception as e:
                    print(f"YouTube fallback error for {title}: {e}", flush=True)
...
def create_film(film: FilmCreate, current_user=Depends(get_current_user)):
    ...
    # YouTube fallback if no trailer found from TMDB
    if not trailer_url:
        yt_key = os.environ.get("YOUTUBE_API_KEY") or os.environ.get("YOUTUBE_KEY", "")
        if yt_key:
            try:
                import urllib.request as _ureq, urllib.parse as _uparse, json as _json
                q = _uparse.urlencode({"part": "snippet", "q": f"{title} {year or ''} official trailer", "type": "video", "maxResults": 1, "key": yt_key})
                with _ureq.urlopen(f"https://www.googleapis.com/youtube/v3/search?{q}", timeout=10) as r:
                    data = _json.loads(r.read())
                items = data.get("items", [])
                if items:
                    video_id = items[0]["id"].get("videoId")
    ...
    with get_db() as conn:
        # Dedup: return existing film if same tmdb_id or same title+year already in DB
        if tmdb_id:
            existing = conn.execute("SELECT id FROM films WHERE tmdb_id=?", (tmdb_id,)).fetchone()
            if existing:
                return get_film_by_id(conn, existing["id"])
        if year:
            existing = conn.execute(
                "SELECT id FROM films WHERE LOWER(title)=LOWER(?) AND year=?", (title, year)
            ).fetchone()
            if existing:
                return get_film_by_id(conn, existing["id"])
        slug = _unique_slug(conn, title)
...
def tmdb_search(q: str, year: Optional[int] = None):
    """Search TMDB (movies) and TVmaze (TV shows) in parallel with trailer enrichment."""
    from concurrent.futures import ThreadPoolExecutor

    def search_tmdb():
        if not os.environ.get("TMDB_API_KEY", "") and not os.environ.get("TMDB_READ_TOKEN", ""):
            print("\n!!! WARNING: TMDB_API_KEY is NOT set. Search results will NOT have trailers. !!!\n", flush=True)
            return []
        ...
        if not raw_results:
            raw_results = [{"id": 550, "title": "Fight Club (Mock)", "release_date": "1999-10-15", "overview": "Mock", "poster_path": None, "backdrop_path": None, "vote_average": 8.8}]
        results = [
            {
                "tmdb_id": r["id"],
                "title": r.get("title"),
        ...
        def enrich_trailer(item):
            # 1. Check local DB
            with get_db() as conn:
                row = conn.execute("SELECT trailer_url FROM films WHERE tmdb_id=?", (item["tmdb_id"],)).fetchone()
                if row and row["trailer_url"]:
                    item["trailerUrl"] = row["trailer_url"]
                    return
            # 2. Fetch from TMDB (no language filter — non-English films only have native trailers)
            v_data = _tmdb._get(f"/movie/{item['tmdb_id']}/videos", {})
            for v in (v_data.get("results") or []):
                if v.get("site") == "YouTube" and v.get("type") == "Trailer":
                    item["trailerUrl"] = f"https://www.youtube.com/embed/{v['key']}"
                    break

        with ThreadPoolExecutor(max_workers=8) as ex:
            list(ex.map(enrich_trailer, results))
        return results

    def search_tvmaze():
        if not os.environ.get("TMDB_API_KEY", "") and not os.environ.get("TMDB_READ_TOKEN", ""):
            print("\n!!! WARNING: TMDB_API_KEY is NOT set. TV show results will NOT have trailers. !!!\n", flush=True)
        ...
        def enrich_trailer(item):
            # 1. Check local DB
            with get_db() as conn:
                row = conn.execute("SELECT trailer_url FROM films WHERE tvmaze_id=?", (item["tvmaze_id"],)).fetchone()
                if row and row["trailer_url"]:
                    item["trailerUrl"] = row["trailer_url"]
                    return
            # 2. Fetch from TMDB TV (Search by title/year)
            trailer, _, _ = _tvmaze._tmdb_tv_trailer_and_stills(item["title"], item["year"])
            if trailer:
...
def bulk_reenrich(token: str = ""):
    ...
    for row in rows:
        ...
        try:
            ...
            result = _tmdb.enrich_film(title, year)
            if result:
                enriched = result
            ...
            results.append({"id": fid, "title": title, "status": "error", "error": str(exc)})
...
def tmdb_details(tmdb_id: int):
    """Return full TMDB enrichment data for a specific TMDB movie ID."""
    if not os.environ.get("TMDB_API_KEY", "") and not os.environ.get("TMDB_READ_TOKEN", ""):
        raise HTTPException(status_code=503, detail="TMDB_API_KEY not configured")
    data = _tmdb.enrich_film_by_id(tmdb_id)
    if not data:
        raise HTTPException(status_code=404, detail="No TMDB match found")
    return data
...
def tv_schedule():
    """Proxy UK TV schedule from TVmaze for the next 7 days, deduplicated by show."""
    ...
    SKIP_TYPES = {"Talk Show", "Game Show", "News", "Variety", "Award Show", "Panel Show"}
    seen = set()
    results = []
    ...
    for offset in range(0, 7):
        ...
        for ep in data:
            ...
            if show_type in SKIP_TYPES:
                continue
            if show_type == "Scripted" and runtime < 45:
                continue
            if show_type not in ("Scripted", "Sports", "Reality", "Documentary", "Animation") and not show_type:
                continue
...
def upcoming_tv():
    ...
    for offset in range(1, 8):
        ...
        for ep in data:
            ...
            show_type = show.get("type", "")
            runtime = show.get("runtime") or show.get("averageRuntime") or 0
            # Accept scripted (any length), reality, sports, documentary — skip news/talk/game shows/soaps
            SKIP_TYPES = {"Talk Show", "Game Show", "News", "Variety", "Award Show", "Panel Show"}
            if show_type in SKIP_TYPES:
                continue
            if show_type not in ("Scripted", "Sports", "Reality", "Documentary", "Animation") and not show_type:
    ...
    with get_db() as conn:
        rows = conn.execute("""
            ...
            WHERE f.poster IS NOT NULL AND f.poster != ''
                AND f.id = (SELECT MIN(f2.id) FROM films f2 WHERE LOWER(f2.title)=LOWER(f.title))
            GROUP BY f.id
...
def trending_all():
    """TMDB trending movies and TV shows today."""
    data = _tvmaze._tmdb_get("/trending/all/day", {"language": "en-US"})
    if not data:
        return []
    tmdb_ids = [s.get("id") for s in (data.get("results") or [])[:20] if s.get("id")]
    # Look up locally stored trailers and streamers for these TMDB IDs
    trailer_map = {}
    streamers_map = {}
    if tmdb_ids:
        with get_db() as conn:
            ...
            rows = conn.execute(
                f"SELECT id, tmdb_id, trailer_url FROM films WHERE tmdb_id IN ({placeholders})",
                tmdb_ids
            ).fetchall()
            film_id_to_tmdb = {}
            for row in rows:
                if row["trailer_url"]:
                    trailer_map[row["tmdb_id"]] = row["trailer_url"]
                film_id_to_tmdb[row["id"]] = row["tmdb_id"]
            if film_id_to_tmdb:
                film_ids = list(film_id_to_tmdb.keys())
                sp = ",".join("?" * len(film_ids))
                streamer_rows = conn.execute(
                    f"SELECT film_id, streamer FROM film_streamers WHERE film_id IN ({sp})",
                    film_ids
                ).fetchall()
                for sr in streamer_rows:
                    tid = film_id_to_tmdb[sr["film_id"]]
                    streamers_map.setdefault(tid, []).append(sr["streamer"])
    results = []
    for s in (data.get("results") or [])[:20]:
        poster = s.get("poster_path")
        ...
        results.append({
            ...
            "trailer_url": trailer_map.get(tmdb_id),
            "streamers": streamers_map.get(tmdb_id, []),
        })
    return results


@app.post("/api/admin/enrich-trending")
def enrich_trending(token: str = Query(...)):
    """Add/refresh all current TMDB trending films/shows so they have streamers + trailers."""
    if token != "reel-admin-2024":
        raise HTTPException(status_code=403, detail="Forbidden")
    data = _tvmaze._tmdb_get("/trending/all/week", {"language": "en-US"})
    if not data:
        return {"error": "TMDB trending fetch failed"}

    def _yt_trailer(title, year):
        yt_key = os.environ.get("YOUTUBE_API_KEY") or os.environ.get("YOUTUBE_KEY", "")
        if not yt_key:
            return ""
        try:
            import urllib.request as _ureq, urllib.parse as _uparse, json as _json
            q = _uparse.urlencode({"part": "snippet", "q": f"{title} {year} official trailer", "type": "video", "maxResults": 1, "key": yt_key})
            with _ureq.urlopen(f"https://www.googleapis.com/youtube/v3/search?{q}", timeout=10) as r:
    ...
    results = {"added": [], "refreshed": [], "errors": []}
    for s in (data.get("results") or [])[:20]:
        tmdb_id = s.get("id")
        is_tv = s.get("media_type") == "tv"
        ...
        try:
            with get_db() as conn:
                existing = conn.execute("SELECT id, trailer_url FROM films WHERE tmdb_id = ?", (tmdb_id,)).fetchone()

            providers = _tmdb.get_watch_providers(tmdb_id, media_type=media_type)

            if existing:
                film_id = existing["id"]
                trailer_url = existing["trailer_url"] or ""
                # Get trailer if missing
                if not is_tv and not trailer_url:
                    enriched = _tmdb.get_film_details(tmdb_id) or {}
                    trailer_url = enriched.get("trailer_url") or _yt_trailer(title, year)
                elif is_tv and not trailer_url:
                    trailer_url = _yt_trailer(title, year)
                with get_db() as conn:
                    if trailer_url and not existing["trailer_url"]:
                        conn.execute("UPDATE films SET trailer_url = ? WHERE id = ?", (trailer_url, film_id))
                    conn.execute("DELETE FROM film_streamers WHERE film_id = ?", (film_id,))
                    for streamer in (providers or []):
                        conn.execute("INSERT OR IGNORE INTO film_streamers (film_id, streamer) VALUES (?, ?)", (film_id, streamer))
                results["refreshed"].append(f"{title} ({year}) [{media_type}] — streamers: {providers}")
            else:
                # Add new entry
                if is_tv:
                    tv_data = _tvmaze._tmdb_get(f"/tv/{tmdb_id}", {"language": "en-US"}) or {}
                    poster_path = tv_data.get("poster_path", "")
                    backdrop_path = tv_data.get("backdrop_path", "")
                    desc = (tv_data.get("overview") or "")[:500]
                    genre = (tv_data.get("genres") or [{}])[0].get("name", "")
                    # Try TMDB videos for trailer
                    videos = _tvmaze._tmdb_get(f"/tv/{tmdb_id}/videos", {"language": "en-US"}) or {}
                    trailer_url = ""
                    ...
                    with get_db() as conn:
                        cur = conn.execute(
                            "INSERT OR IGNORE INTO films (title, year, description, poster, backdrop, trailer_url, tmdb_id, genre) VALUES (?,?,?,?,?,?,?,?)",
                            (title, year, desc,
                    ...
                    with get_db() as conn:
                        cur = conn.execute(
                            "INSERT OR IGNORE INTO films (title, year, description, poster, backdrop, trailer_url, tmdb_id, genre, director, cast, runtime) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
                            (enriched.get("title", title), enriched.get("year", year),
                             enriched.get("description", ""), enriched.get("poster_url"),
                             enriched.get("backdrop_url"), trailer_url, tmdb_id,
                             enriched.get("genre", ""), enriched.get("director", ""),
                             enriched.get("cast", ""), enriched.get("runtime", ""))
                        )
                        film_id = cur.lastrowid
                        for streamer in (providers or []):
                            conn.execute("INSERT OR IGNORE INTO film_streamers (film_id, streamer) VALUES (?, ?)", (film_id, streamer))
                results["added"].append(f"{title} ({year}) [{media_type}] — streamers: {providers}, trailer: {'yes' if trailer_url else 'no'}")
        except Exception as e:
...
def record_recommendation(body: dict, current_user=Depends(require_user)):
    ...
    with get_db() as conn:
        ...
        if film_id:
            ...
            try:
                y = int(film_year) if film_year else None
                if y:
                    film = conn.execute("SELECT id FROM films WHERE LOWER(title)=LOWER(?) AND year=?", (film_title, y)).fetchone()
                if not film:
                    film = conn.execute("SELECT id FROM films WHERE LOWER(title)=LOWER(?)", (film_title,)).fetchone()
            except Exception:
                film = conn.execute("SELECT id FROM films WHERE LOWER(title)=LOWER(?)", (film_title,)).fetchone()
        
        if not film and film_title:
            # Auto-create the film record if it's missing
            try:
                enriched = _tmdb.enrich_film(film_title, int(film_year) if film_year else None)
            except Exception:
                enriched = {}
            
            def pick(manual, key):
                return manual if manual else enriched.get(key)
            
            title = film_title
            year = int(film_year) if film_year else enriched.get("year")
            genre = pick(genre, "genre")
            runtime = pick(runtime, "runtime")
            director = enriched.get("director")
            cast = enriched.get("cast")
            f_rating = enriched.get("rating")
            poster = pick(poster, "poster")
            backdrop = enriched.get("backdrop") or poster
            trailer_url = pick(trailer, "trailer_url")
            ...
            cur = conn.execute(
                """INSERT INTO films 
                   (title, year, genre, runtime, director, cast, rating, shared_by, note,
                    poster, backdrop, trailer_url, description, tmdb_id, slug)
                   VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (title, year, genre, runtime, director, cast, f_rating, from_username or "A friend", note,
                 poster, backdrop, trailer_url, description, tmdb_id, slug)
            )
            fid = cur.lastrowid
            # Also add stills/streamers from enrichment if available
            for s in enriched.get("streamers", []):
                conn.execute("INSERT INTO film_streamers (film_id, streamer) VALUES (?,?)", (fid, s))
            for s in enriched.get("stills", []):
                conn.execute("INSERT INTO film_stills (film_id, still_url) VALUES (?,?)", (fid, s))
...
def send_recommendation(body: dict, current_user=Depends(require_user)):
    ...
    with get_db() as conn:
        to_user = conn.execute(
            "SELECT id, email, display_name, username FROM users WHERE username=?", (to_username,)
        ).fetchone()
        if not to_user:
            raise HTTPException(status_code=404, detail="User not found")
        film = conn.execute(
            "SELECT id, title, year, poster, backdrop FROM films WHERE id=?", (film_id,)
        ).fetchone()
        if not film:
            raise HTTPException(status_code=404, detail="Film not found")
        try:
            conn.execute(
                "INSERT OR REPLACE INTO user_recommendations (film_id, from_user_id, to_user_id, note, rating) VALUES (?,?,?,?,?)",
                (film_id, current_user["id"], to_user["id"], note or None, rating or None)
            )
        except Exception as e:
            raise HTTPException(status_code=400, detail=str(e))
    from_name = current_user.get("display_name") or current_user.get("username", "A friend")
    to_name = to_user["display_name"] or to_user["username"]
    poster = film["backdrop"] or film["poster"] or ""
    _send_recommendation_email(
...
def admin_backup_users(token: str = ""):
    """Export all user-related data as JSON backup before a wipe."""
    _check_admin(token)
    with get_db() as conn:
        users = [dict(r) for r in conn.execute("SELECT * FROM users").fetchall()]
        user_friends = [dict(r) for r in conn.execute("SELECT * FROM user_friends").fetchall()]
        user_watchlist = [dict(r) for r in conn.execute("SELECT * FROM user_watchlist").fetchall()]
        user_watched = [dict(r) for r in conn.execute("SELECT * FROM user_watched").fetchall()]
        user_ratings = [dict(r) for r in conn.execute("SELECT * FROM user_ratings").fetchall()]
        user_recommendations = [dict(r) for r in conn.execute("SELECT * FROM user_recommendations").fetchall()]
        watchlist = [dict(r) for r in conn.execute("SELECT * FROM watchlist").fetchall()]
        watched = [dict(r) for r in conn.execute("SELECT * FROM watched").fetchall()]
        friends = [dict(r) for r in conn.execute("SELECT * FROM friends").fetchall()]
        friend_watched = [dict(r) for r in conn.execute("SELECT * FROM friend_watched").fetchall()]
    return {
        "backup_at": __import__("datetime").datetime.utcnow().isoformat(),
        "users": users,
        ...
        "watched": watched,
        "friends": friends,
        "friend_watched": friend_watched,
    }


@app.delete("/api/admin/wipe-users")
def admin_wipe_users(token: str = ""):
    """Delete all users and all their associated data. Films are untouched."""
    _check_admin(token)
    with get_db() as conn:
        conn.execute("DELETE FROM user_recommendations")
        conn.execute("DELETE FROM user_ratings")
        conn.execute("DELETE FROM user_watched")
        conn.execute("DELETE FROM user_watchlist")
        conn.execute("DELETE FROM user_friends")
        conn.execute("DELETE FROM friend_watched")
        conn.execute("DELETE FROM watched")
        conn.execute("DELETE FROM watchlist")
        conn.execute("DELETE FROM friends")
        conn.execute("DELETE FROM search_logs")
        conn.execute("DELETE FROM tab_views")
        conn.execute("DELETE FROM users")
    return {"status": "ok", "message": "All users and user data deleted. Films intact."}


@app.patch("/api/admin/films/{film_id}")
def admin_patch_film(film_id: int, body: dict, token: str = ""):
    """Patch any field(s) on a film. Allowed: trailer_url, tmdb_id, poster, backdrop, genre, description, cast, director, runtime, title, year, rating, clear_stills."""
    _check_admin(token)
    ALLOWED = {"trailer_url", "tmdb_id", "poster", "backdrop", "genre", "description", "title", "year", "rating", "cast", "director", "runtime"}
    QUOTED = {"cast"}  # SQL reserved keywords that must be quoted
    updates = {k: v for k, v in body.items() if k in ALLOWED}
    with get_db() as conn:
        if updates:
            sets = ", ".join(f'"{k}"=?' if k in QUOTED else f"{k}=?" for k in updates)
            conn.execute(f"UPDATE films SET {sets} WHERE id=?", (*updates.values(), film_id))
        if body.get("clear_stills"):
            conn.execute("DELETE FROM film_stills WHERE film_id=?", (film_id,))
        return get_film_by_id(conn, film_id)


@app.post("/api/admin/normalize-ratings")
def normalize_ratings(token: str = ""):
    """Halve any ratings stored on 0-10 scale (>5) to bring everything to 0-5."""
    _check_admin(token)
    with get_db() as conn:
        rows = conn.execute("SELECT id, title, rating FROM films WHERE rating > 5").fetchall()
        updated = []
        for row in rows:
            new_rating = round(row["rating"] / 2, 2)
            conn.execute("UPDATE films SET rating=? WHERE id=?", (new_rating, row["id"]))
            updated.append({"id": row["id"], "title": row["title"], "old": row["rating"], "new": new_rating})
    return {"updated": len(updated), "films": updated}


@app.delete("/api/admin/dedup-films")
def admin_dedup_films(token: str = ""):
    """Delete duplicate films (same title), keeping the one with the lowest id."""
    _check_admin(token)
    deleted = []
    with get_db() as conn:
        dupes = conn.execute("""
        ...
        for row in dupes:
            keep_id = row["keep_id"]
            dupe_ids = [r["id"] for r in conn.execute(
                "SELECT id FROM films WHERE LOWER(title)=LOWER(?) AND id != ?",
                (row["title"], keep_id)
            ).fetchall()]
            for did in dupe_ids:
                conn.execute("DELETE FROM film_streamers WHERE film_id=?", (did,))
                conn.execute("DELETE FROM film_stills WHERE film_id=?", (did,))
                # Move watchlist/watched/rec rows to the kept film — INSERT OR IGNORE handles existing dupes
                conn.execute("INSERT OR IGNORE INTO user_watchlist (user_id, film_id, added_at) SELECT user_id, ?, added_at FROM user_watchlist WHERE film_id=?", (keep_id, did))
                conn.execute("DELETE FROM user_watchlist WHERE film_id=?", (did,))
                conn.execute("INSERT OR IGNORE INTO user_watched (user_id, film_id, watched_at) SELECT user_id, ?, watched_at FROM user_watched WHERE film_id=?", (keep_id, did))
                conn.execute("DELETE FROM user_watched WHERE film_id=?", (did,))
                conn.execute("INSERT OR IGNORE INTO user_recommendations (film_id, from_user_id, to_user_id, note, rating) SELECT ?, from_user_id, to_user_id, note, rating FROM user_recommendations WHERE film_id=?", (keep_id, did))
                conn.execute("DELETE FROM user_recommendations WHERE film_id=?", (did,))
                conn.execute("DELETE FROM films WHERE id=?", (did,))
                deleted.append({"title": row["title"], "removed_id": did, "kept_id": keep_id})
...
@app.get("/api/teleprompter", include_in_schema=False)
def get_teleprompter_script():
    if os.path.exists(TELEPROMPTER_JSON):
        with open(TELEPROMPTER_JSON, "r", encoding="utf-8") as f:
            return json.load(f)
    return {"script": "", "updated_at": ""}

@app.post("/api/teleprompter", include_in_schema=False)
def save_teleprompter_script(data: TeleprompterUpdate):
    existing = {}
    if os.path.exists(TELEPROMPTER_JSON):
        with open(TELEPROMPTER_JSON, "r", encoding="utf-8") as f:
            existing = json.load(f)
    existing.update({"script": data.script, "updated_at": datetime.utcnow().isoformat()})
    if data.speed is not None:
        existing["speed"] = data.speed
    if data.font_size is not None:
        existing["font_size"] = data.font_size
    with open(TELEPROMPTER_JSON, "w", encoding="utf-8") as f:
        json.dump(existing, f, ensure_ascii=False)
    return existing

@app.get("/teleprompter", include_in_schema=False)
def serve_teleprompter():
    return FileResponse(TELEPROMPTER_FILE)
