...
export default function DiscoverTab({ allFilms, addedIds, onOpenFilm, onAddToList, providers }) {
  ...
  const scoreKey = timeRange === 'Week' ? 'activityScore7d'
    : timeRange === 'Month' ? 'activityScore30d'
    : timeRange === 'Year' ? 'activityScore365d'
    : 'activityScore'
  ...
  }, [allFilms, genreFilter, providerFilter, scoreKey])
  ...
  return (
    <div style={{
      ...
      <div className="no-scrollbar" style={{ height: '100%', overflowY: 'auto' }}>
        {/* Header */}
        <div style={{ padding: '14px 16px 2px', fontSize: '13px', fontWeight: '800', color: 'rgb(255, 255, 255)', letterSpacing: '1.5px', textTransform: 'uppercase' }}>🔥 What's trending with Watch Chums users</div>
          ...
          <div onClick={() => onOpenFilm(heroFilm)} style={{ margin: '0px 16px 20px', borderRadius: '16px', overflow: 'hidden', position: 'relative', height: '220px', cursor: 'pointer' }}>
            ...
            <div style={{ position: 'absolute', bottom: '0px', left: '0px', right: '0px', padding: '0px 14px 14px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                ...
                <span style={{ fontSize: '12px', color: 'rgba(255, 255, 255, 0.8)' }}>{(heroFilm.rating || 0).toFixed(1)}</span>
            ...
            <div key={film.id} onClick={() => onOpenFilm(film)} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 16px', borderBottom: '1px solid rgba(255, 255, 255, 0.05)', cursor: 'pointer' }}>
              ...
              <div style={{ flex: '1 1 0%', minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '3px' }}>
                  ...
                  <span style={{ fontSize: '10px', color: 'var(--gold-bright)', fontWeight: '700' }}>{(film.rating || 0).toFixed(1)}/5</span>
                </div>
              </div>
              <button
