...
export default function FilmDetailPage({
  ...
  const hasTrailer = !!(film.trailer_url || film.trailerUrl)

  function playTrailer() {
    const url = film.trailer_url || film.trailerUrl
    if (url) {
      onWatchTrailer && onWatchTrailer(url)
    }
  }

  // Format rating to show stars — rating is stored 0-5 (TMDB 0-10 / 2)
  const renderStars = (rating) => {
    ...
    return (
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: '2px' }}>
        ...
        <span style={{ fontSize: '10px', color: 'var(--text2)', marginLeft: '3px' }}>{rating.toFixed(1)}/5</span>
      </span>
    )
  }
    ...
    <div style={{
      ...
      <div className="mobile-view" style={{ paddingBottom: '64px' }}>
        ...
        <div style={{ position: 'relative', height: '230px', flexShrink: 0 }}>
          ...
          {(film.trailer_url || film.trailerUrl) && (
        ...
        <div style={{ padding: '4px 18px 36px', maxWidth: '680px', margin: '0px auto', width: '100%', boxSizing: 'border-box' }}>
          ...
          <div style={{ marginBottom: '6px' }}>
            {renderStars(userRatings?.[film.id] ?? film.rating)}
          </div>
      ...
      <div className="desktop-view" style={{ display: 'none' }}>
        ...
        <div className="glass-card" style={{ marginTop: '40px' }}>
          <div className="desktop-poster-wrapper">
            <div style={{ position: 'relative', cursor: (film.trailer_url || film.trailerUrl) ? 'pointer' : 'default' }} onClick={playTrailer}>
          ...
          >
            ...
            {selectedRating === 0 ? (
              ...
              <div style={{ display: 'flex', gap: '10px', marginTop: '4px' }}>
                <button
                  onClick={async () => {
                    const resolvedId = await onMarkWatched(film)
                    onSaveRating(resolvedId || film.id, selectedRating)
                    onRecommend(film)
                    setShowRatingModal(false)
                    setSelectedRating(0)
                  }}
