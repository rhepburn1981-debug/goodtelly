...
export default function WatchlistDashboard(props) {
    ...
    const dynamicGenres = React.useMemo(() => {
        ...
        myList.forEach(item => {
            ...
            if (film.genre) set.add(film.genre.split(',')[0].trim());
        });
    ...
    const displayFilms = baseList
        .filter(item => {
            const film = item.film || item;
            if (activeGenre !== 'All' && (film.genre || '').split(',')[0].trim().toLowerCase() !== activeGenre.toLowerCase()) return false;
            if (activeService !== 'All' && !(film.streamers || []).includes(activeService)) return false;
                            ...
                            {displayFilms.map(item => {
                                ...
                                return (
                                    <div key={filmId} className="wl-card" onClick={() => onOpenFilm(film)} style={{ cursor: 'pointer' }}>
                                        <div style={{ position: 'relative', borderRadius: 20, overflow: 'hidden', border: '1px solid #FFFFFF33' }}>
                                            <img
                                        ...
                                        <div style={{ padding: '10px 11px 0' }}>
                                            ...
                                            <button
                                                ...
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    isWatched ? onUnmarkWatched(film) : (onSeenIt ? onSeenIt(film) : onMarkWatched(film));
                                                }}
                                            >
