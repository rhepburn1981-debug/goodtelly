...
import { FaPlay, FaPlus, FaStar, FaUserFriends, FaChevronRight, FaChevronLeft } from 'react-icons/fa';
        ...
        <div style={{
            ...
            <div style={{ position: 'relative', zIndex: 2, width: '100%', display: 'flex', gap: '20px' }}>
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                    <div style={{ fontSize: '14px', color: '#E2C367', fontWeight: '600', textTransform: 'uppercase', marginBottom: '10px' }}>
                        {displayRec.rec_from_name || displayRec.rec_from_username || "A Friend"}'s Recommending
                    </div>
...
const HorizontalScroller = ({ children, gap = 25 }) => {
    ...
    return (
        <div style={{ position: 'relative', width: '100%' }}>
            {showLeft && (
                <div
                    ...
                    style={{
                        ...
                        boxShadow: '0 4px 15px rgba(0,0,0,0.5)', transition: 'all 0.2s ease'
                    }}
                    onMouseEnter={e => e.currentTarget.style.background = '#000'}
                    onMouseLeave={e => e.currentTarget.style.background = 'rgba(20, 20, 20, 0.9)'}
                >
            ...
            <div
                ...
                style={{ display: 'flex', gap, overflowX: 'auto', paddingBottom: 24, scrollSnapType: 'x mandatory' }}
...
const StandardCard = ({ img, label, year, streamers, onClick }) => {
    const firstStreamer = (streamers || []).find(s => STREAMER_LOGOS[s]);
    const logo = firstStreamer ? STREAMER_LOGOS[firstStreamer] : null;
            ...
            <div className="hover-card" style={{ position: 'relative', borderRadius: '18px', overflow: 'hidden', marginBottom: '12px', border: '1px solid rgba(255,255,255,0.08)', height: '260px' }}>
                ...
                {logo && (
                    <div style={{
                        ...
                        <img src={logo.src} alt={firstStreamer} style={{ width: logo.width, height: 'auto', objectFit: 'contain' }} />
                    </div>
                )}
            </div>
...
const STREAMER_LOGOS = {
    'Netflix': { src: '/branding/netflix.png', width: '22px' },
    'Prime Video': { src: '/branding/prime.png', width: '36px' },
    'Disney+': { src: '/branding/disney.png', width: '36px' },
    'Apple TV+': { src: '/branding/I-tv.png', width: '38px' },
    'NOW TV': { src: '/branding/now_logo.png', width: '36px' },
    'Paramount+': { src: '/branding/paramountplus.png', width: '36px' },
    'Discovery+': { src: '/branding/discovery.png', width: '36px' },
    'ITVX': { src: '/branding/itvx.svg', width: '36px' },
    'BBC iPlayer': { src: '/branding/bbc.png', width: '36px' },
    'Channel 4': { src: '/branding/channel4.png', width: '30px' },
};

const RankedPoster = ({ num, img, title, year, streamers, onClick }) => {
    const firstStreamer = (streamers || []).find(s => STREAMER_LOGOS[s]);
    const logo = firstStreamer ? STREAMER_LOGOS[firstStreamer] : null;
...
const OnTVCard = ({ img, title, date, provider, onClick }) => (
    <div onClick={onClick} style={{ cursor: 'pointer', flexShrink: 0, width: 250 }}>
        <div className="hover-card" style={{ position: 'relative', overflow: 'hidden', marginBottom: '10px', height: '300px' }}>
            <img src={img} alt={title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
        </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '32px', marginTop: '10px' }}>
                    ...
                    {trendingNow.length > 0 && (
                        <div style={{
                            ...
                            <div style={{ fontSize: '20px', fontWeight: '800', color: '#fff', marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                                <div style={{
                                    position: 'relative', width: '24px', height: '24px',
                                    display: 'flex', alignItems: 'center', justifyContent: 'center'
                                }}>
                                    <svg viewBox="0 0 24 24" width="24" height="24" fill="none" style={{ filter: 'drop-shadow(0 0 6px #3b82f6)' }}>
                                ...
                                <span style={{ color: '#E2C367' }}>What’s NEW</span>
                            </div>
                            <HorizontalScroller>
                                {trendingNow.slice(0, 20).map((show, idx) => (
                                    <RankedPoster
                                        key={show.id}
                                        num={idx + 1}
                                        img={show.poster || show.poster_url}
                    ...
                    {/* Trending with Watch Chums Users (API Data) */}
                        ...
                        <div style={{
                            ...
                            <div style={{ fontSize: '20px', fontWeight: '800', color: '#fff', marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                                <svg viewBox="0 0 24 24" width="22" height="22" fill="none">
                                    <path d="M3 17L9 11L13 15L21 7" stroke="url(#arrowG)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"></path>
                                    <path d="M15 7h6v6" stroke="url(#arrowG)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"></path>
                                    <defs>
                                ...
                                Trending with Watch Chums Users
                            </div>
                            <HorizontalScroller>
                                {watchlistRecent.map((show, idx) => (
                                    <StandardCard
                                        ...
                                        year={show.year || (show.release_date ? show.release_date.split('-')[0] : '2024')}
                                        streamers={show.streamers}
                                        onClick={() => onOpenFilm(show)}
                                    />
                                ))}
                    ...
                    {upcomingShows.length > 0 && (
                        <div style={{
                            ...
                            <div style={{ fontSize: '20px', fontWeight: '800', color: '#fff', marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                                <svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="#e2b644" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                            ...
                            <HorizontalScroller>
                                {upcomingShows.slice(0, 14).map((show, idx) => (
                                    <OnTVCard
                                        key={show.id || idx}
                                        img={show.image || '/branding/poster1.png'}
                                        title={show.name}
                                        date={`${formatDate(show.airdate)} • ${show.airtime || '21:00'}`}
                                        provider={show.network?.name || 'TV'}
