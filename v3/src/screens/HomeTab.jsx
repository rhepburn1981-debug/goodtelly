    ...
    <div style={{ margin: '2px 16px 10px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '0px 2px 6px' }}>
        ...
        <div style={{ fontSize: '13px', color: 'var(--gold-bright)', fontWeight: '500' }}>
          {displayRec.rec_from_name || displayRec.rec_from_username || displayRec.from_display_name || displayRec.from_username || 'A Friend'} thinks you&rsquo;ll like&hellip;
        </div>
      </div>
    ...
    <div style={{
      ...
      {recentWatchlist.length > 0 && <div>
        ...
        <div style={{ display: 'flex', gap: '10px', overflowX: 'auto', padding: '0px 16px 12px', scrollbarWidth: 'none' }} className="no-scrollbar">
          {recentWatchlist.map((film, idx) => (
            ...
            >
              <img
                src={film.poster || film.poster_url}
                alt={film.title || film.name}
      ...
      <div style={{ paddingBottom: '24px' }}>
        ...
        <div style={{ margin: '0px 16px', background: 'rgba(255, 255, 255, 0.02)', borderRadius: '20px', border: '1px solid rgba(255, 255, 255, 0.05)', overflow: 'hidden' }}>
          {(upcomingShows.length > 0 ? upcomingShows.slice(0, 14) : []).map((item, idx) => (
            <div
              key={item.id || idx}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '2px' }}>
                  <div style={{
                    ...
                    ...getChannelStyles(item.channel || item.network?.name || 'TV')
                  }}>
                    {item.channel || item.network?.name || 'TV'}
                  </div>
