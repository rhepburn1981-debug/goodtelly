...
export default function RecommendModal({ film, currentUser, friends = [], onClose, onToast }) {
  const [note, setNote] = useState('')
  const [rating, setRating] = useState(0)
  const [sending, setSending] = useState(false)
  const [sentFriends, setSentFriends] = useState([]) // Track which friends got in-app recs
  ...
  async function ensureInternalId() {
    // If it's an external film (TMDB/TVMaze) and we don't have a small internal ID, add it
    if (film._isExternal || film._fromTmdb) {
      // If film.id is large (>100k), it's likely a TMDB ID
      // The backend addFilm will check if it already exists or create it
      try {
        const added = await addFilm({
          title: film.title,
          year: film.year ? parseInt(film.year) : null,
          tmdbId: film.tmdb_id || film.id,
          autoEnrich: true
        });
        return added.id;
      } catch (e) {
        console.error("Failed to add film to internal DB", e);
        return film.id; // Fallback to current ID
      }
    }
    return film.id;
  }
  ...
  async function handleSendGlobal() {
    ...
    try {
      ...
      await sendRecommendation(null, internalId, note, rating || null)
    } catch (_) { }
  ...
  async function handleSendInApp(friend) {
    if (sentFriends.includes(friend.username)) return
    try {
      const internalId = await ensureInternalId();
      await sendRecommendation(friend.username, internalId, note, rating || null)
      setSentFriends(prev => [...prev, friend.username])
      onToast && onToast(`Recommended to ${friend.username}!`)
    } catch (_) {
      onToast && onToast('Failed to send in-app')
    }
  }

  function handleSendWhatsApp(friend) {
    // For specific friend, we still use the global share but can add context if needed
    sendViaWhatsApp(shareUrl, film, (currentUser?.display_name || currentUser?.username || 'A friend'), note, rating)
  }
        ...
        <div style={{ overflowY: 'auto', padding: '14px 16px 8px', flex: '1 1 0%' }} className="no-scrollbar">
          ...
          <input
            ...
            style={{
              ...
              fontFamily: 'var(--ff-body)', fontStyle: 'italic', marginBottom: '20px'
            }}
          />
          ...
          {friends.length > 0 && (
            <div style={{ marginBottom: '20px' }}>
              <div style={{ fontSize: '10px', fontWeight: '700', color: 'rgb(63, 118, 255)', textTransform: 'uppercase', letterSpacing: '1.5px', marginBottom: '14px' }}>Friends</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {friends.map((friend) => (
                  <div key={friend.username} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <div style={{
                      width: '36px', height: '36px', borderRadius: '50%',
                      background: 'rgba(255, 255, 255, 0.1)',
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      fontSize: '14px', fontWeight: '800', color: '#fff', overflow: 'hidden'
                    }}>
                    ...
                    <div style={{ flex: 1, fontSize: '14px', fontWeight: '600', color: '#fff', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {friend.display_name || friend.username}
                    </div>
                    <div style={{ display: 'flex', gap: '6px' }}>
                      <button
                        onClick={() => handleSendInApp(friend)}
                        disabled={sentFriends.includes(friend.username)}
                        style={{
                          padding: '6px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: '700',
                          background: sentFriends.includes(friend.username) ? 'rgba(255, 255, 255, 0.05)' : 'rgba(63, 118, 255, 0.12)',
                          border: sentFriends.includes(friend.username) ? '1px solid rgba(255, 255, 255, 0.1)' : '1px solid rgba(63, 118, 255, 0.3)',
                          color: sentFriends.includes(friend.username) ? 'rgba(255, 255, 255, 0.3)' : 'rgb(63, 118, 255)',
                          cursor: sentFriends.includes(friend.username) ? 'default' : 'pointer',
                          transition: '0.2s'
                        }}
                      >
                        {sentFriends.includes(friend.username) ? 'Sent' : 'In-app'}
                      </button>
                      <button
                        onClick={() => handleSendWhatsApp(friend)}
                        style={{
                          padding: '6px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: '700',
                          background: 'rgba(37, 211, 102, 0.08)', border: '1px solid rgba(37, 211, 102, 0.25)',
                          color: 'rgb(37, 211, 102)', cursor: 'pointer'
                        }}
                      >
                        WhatsApp
                      </button>
