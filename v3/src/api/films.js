...
export function getFilms() {
  return api.get('/api/films').then((films) => (films || []).map(normalizeFilm))
}
...
export function searchTmdb(query) {
  return api.get('/api/tmdb/search?q=' + encodeURIComponent(query))
    .then((results) => (results || []).map((r) => ({ ...r, _fromTmdb: true })))
}

export function getTrending() {
  return api.get('/api/trending/all')
    .then((items) => (items || []).map(normalizeExternal))
}

export function getWatchlistRecent() {
  return api.get('/api/trending/watchlist-recent')
    .then((items) => (items || []).map(normalizeFilm))
}

export function getUpcomingTv() {
  return api.get('/api/trending/upcoming-tv')
    .then((items) => (items || []).map(normalizeExternal))
}

export function getTvSchedule() {
  return api.get('/api/trending/tv-schedule')
    .then((items) => (items || []).map(normalizeExternal))
}

export function getDiscoverTv() {
  return api.get('/api/trending/discover-tv')
    .then((items) => (items || []).map(normalizeExternal))
}

export function getProviders() {
  return api.get('/api/providers').catch(() => [])
}
